...
# Loaddata from train dataset   
petFinder <- read.csv("train.csv")

# Remove columns that are hard to be useful (string and random ID) -- Name, RescuerID, Description, PetID
petFinder <- petFinder[, -c(2, 19, 21, 22)]
...
str(petFinder)
# 20 variables, all numeric and factor
...
x <- model.matrix(AdoptionSpeed~., train.petFinder)[ ,-20]
...
x2 <- model.matrix(AdoptionSpeed~., test.petFinder)[ ,-20]
